...
use classicmodels;
-- Q.1)List all details of customers whose credit limit is greater than the average credit limit of their country.
select * from customers;

desc customers; 

select * from customers group by country having creditLimit>avg(creditLimit);


-- Q.2)List out the office code, the city and the cumulative distribution (rounded to 3 decimal points) of employees ordered by their office code.
select officeCode,city,cume_dist() over(order by officeCode) from offices;


-- Q.3)List customers whose representatives report to Mr. William Patterson, ranked based on their credit limit.
...
-- Q.4)Determine the customer with the highest credit limit.
select customerNumber,customerName,contactLastName,contactFirstName,creditLimit ,RANK() over(Order by creditLimit desc) from
customers limit 1;

-- Q.5)What was the cost of the query for the previous question (d)? What will be the cost if the query is rewritten
